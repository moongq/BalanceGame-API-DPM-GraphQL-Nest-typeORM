...
let BalanceGameService = class BalanceGameService {
    constructor(balanceGameRepository, userRepository, balanceGameSelectionVote, voteRepository, balanceGameKeywordService, balanceGameSelectionService, fileService) {
        this.balanceGameRepository = balanceGameRepository;
        this.userRepository = userRepository;
        this.balanceGameSelectionVote = balanceGameSelectionVote;
    ...
    async findAllLogined(userId, limit, offset) {
        const [balanceGames, count] = await this.balanceGameRepository.findAndCount({
            relations: ["balanceGameKeywords", "balanceGameSelections"],
            take: limit,
            skip: offset,
    ...
    async findOne(userId, gameId) {
        const result = await this.balanceGameRepository.findOne({ id: gameId }, { relations: ["balanceGameKeywords", "balanceGameSelections", "user", "user.profile"] });
        if (!result) {
            throw new common_1.HttpException("Wrong id input/gameId", common_1.HttpStatus.BAD_REQUEST);
        }
        const myGameWithSelection = await this.voteRepository
    ...
    async findOneByRandom(userId) {
        ...
        const game = await this.balanceGameRepository.findOne({ id: result[index].id }, { relations: ["balanceGameKeywords", "balanceGameSelections"] });
        return game;
    }
    ...
    async myVotedGames(userId) {
        ...
        if (gameArray.length == 0) {
            throw new common_1.HttpException("dont have voted games", common_1.HttpStatus.BAD_REQUEST);
        }
        let getOnlyGameIds = [];
        ...
        const games = await this.balanceGameRepository.find({
            where: { id: typeorm_2.In(getOnlyGameIds) },
...
BalanceGameService = __decorate([
    ...
    __param(3, typeorm_1.InjectRepository(balance_game_selection_vote_model_1.BalanceGameSelectionVote)),
